...
echo "Incluye limpieza de SVRCODE NETVPN Manager si está instalado."
echo ""
...
cp -a /etc/svrcode-netvpn "$BACKUP_DIR/svrcode-netvpn-etc" 2>/dev/null || true
cp -a /var/lib/svrcode-netvpn "$BACKUP_DIR/svrcode-netvpn-data" 2>/dev/null || true
...
echo "== Desinstalando SVRCODE NETVPN Manager si existe =="
if command -v svrnetvpn >/dev/null 2>&1; then
  svrnetvpn uninstall 2>/dev/null || true
fi
...
rm -f /etc/systemd/system/svrcode-netvpn-api.service
rm -f /etc/systemd/system/svrcode-token-api.service
...
rm -f /usr/local/bin/svrnetvpn
rm -f /usr/local/bin/svrtoken
...
rm -rf /opt/svrcode-netvpn
rm -rf /etc/svrcode-netvpn
rm -rf /var/lib/svrcode-netvpn
rm -f /var/log/svrcode-netvpn-api.log /var/log/svrcode-netvpn-auth.log
